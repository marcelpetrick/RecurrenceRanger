...
from recurrence_ranger.normalize import parse_record
...
class Collector:
    ...
    def _source(self, source: Source) -> None:
        ...
        """, (source.id, source.tool, source.label, str(source.home), source.origin,
              int(source.exists), utc_now()))
    ...
    def _error(self, source: Source, path: Path, stage: str, detail: str) -> None:
        with self.store.db:
            self.store.db.execute("INSERT INTO errors(source_id,path,stage,detail,created_at) "
                                  "VALUES (?,?,?,?,?)",
    ...
    def _new_generation(self, file_id: int, previous: tuple | None, stat, reason: str) -> tuple:
        ...
        if previous:
            db.execute("UPDATE pending_fragments SET status='abandoned' "
                       "WHERE generation_id=? AND status='pending'", (previous[0],))
        cursor = db.execute("""
          INSERT INTO generations(file_id,number,device,inode,reason,created_at,
                                  last_size,last_mtime_ns)
          VALUES (?,?,?,?,?,?,?,?)
        """, (file_id, number, stat.st_dev, stat.st_ino, reason, utc_now(),
              stat.st_size, stat.st_mtime_ns))
        db.execute("UPDATE files SET current_generation=? WHERE id=?", (cursor.lastrowid, file_id))
        return (cursor.lastrowid, number, stat.st_dev, stat.st_ino, 0, "",
                stat.st_size, stat.st_mtime_ns)
                ...
                with self.store.db:
                    ...
                    while handle.tell() < stat.st_size:
                        ...
                        cursor = self.store.db.execute("""
                          ...
                          VALUES (?,?,?,?,?,?,?,?)
                        """, (generation_id, start, end, line, digest, utc_now(), status, error))
                        ...
                        if result.bytes >= budget:
                            break
                    self.store.db.execute("UPDATE generations SET checkpoint=?,fingerprint=?,"
                                          "last_size=?,last_mtime_ns=? WHERE id=?",
                                          (offset, _fingerprint(handle, offset), stat.st_size,
                                           stat.st_mtime_ns, generation_id))
                    self.store.db.execute("DELETE FROM pending_fragments "
                                          "WHERE generation_id=? AND status='pending' "
                                          "AND start_offset<?", (generation_id, offset))
    ...
    def _message(self, record_id: int, source: Source, message) -> None:
        ...
        session_id = db.execute("SELECT id FROM sessions WHERE source_id=? AND "
                                "source_session_id=?", (source.id, message.session)).fetchone()[0]
        db.execute("""
          INSERT INTO messages(record_id,session_id,source_message_id,role,kind,
                               timestamp,project,text,parent_id)
          VALUES (?,?,?,?,?,?,?,?,?)
        """, (record_id, session_id, message.source_message_id, message.role,
              message.kind, message.timestamp, message.project, message.text,
              message.parent_id))
        db.executemany("INSERT INTO content_blocks VALUES (?,?,?,?,?)", [
    ...
    def scan_once(self, sources: list[Source], *, max_files: int = 32) -> ScanResult:
        ...
        rows = self.store.db.execute("""
          SELECT f.path,g.checkpoint,g.last_size,g.last_mtime_ns,p.status
        ...
        for source, path, stat in paths:
            ...
            if not state or state[2] != stat.st_size or state[3] != stat.st_mtime_ns:
                needed.append((source, path, stat))
            elif state[0] < stat.st_size and state[4] != "pending":
                needed.append((source, path, stat))
        recent = sorted((row for row in needed if time.time() - row[2].st_mtime < 3600),
                        key=lambda row: row[2].st_mtime, reverse=True)
        historical = sorted((row for row in needed if time.time() - row[2].st_mtime >= 3600),
